	...
	private static void initMimeTypes() {
		...
		try {
			...
			try {
				...
				CapoApplication.logger.log(Level.SEVERE,"Error loading internal mime-types.properties", e);
			}finally {
				is = closeStream(is);
			}
			...
			try {
				...
				if (f.exists()) {
					...
					if (is != null) {
						CapoApplication.logger.fine("Found a custom .mime-types.properties file in the users home directory.");
						Properties props = new Properties();
						props.load(is);
						if (props.size() > 0) {
							extMimeTypes.putAll(props);
						}
						CapoApplication.logger.fine("Successfully parsed .mime-types.properties from users home directory.");
					}
				}
			} catch (Exception e) {
				CapoApplication.logger.log(Level.SEVERE,"Failed to parse .magic.mime file from users home directory. File will be ignored.",e);
			} finally {
				is = closeStream(is);
			}
			...
			try {
				...
				while(e.hasMoreElements()) {
					URL url = (URL)e.nextElement();
						...
						CapoApplication.logger.fine("Found custom mime-types.properties file on the classpath [" + url + "].");
						...
						if(is != null) {
							...
							if(props.size() > 0) {
								extMimeTypes.putAll(props);
									...
									CapoApplication.logger.fine("Successfully loaded custome mime-type.properties file [" + url + "] from classpath.");
								
							}
						}
					}catch(Exception ex) {
						CapoApplication.logger.log(Level.SEVERE,"Failed while loading custom mime-type.properties file [" + url + "] from classpath. File will be ignored.");
					}
				}
			}catch(Exception e) {
				CapoApplication.logger.log(Level.SEVERE,"Problem while processing mime-types.properties files(s) from classpath. Files will be ignored.", e);
			} finally {
				is = closeStream(is);
			}
			...
			try {
				...
				if (fname != null && fname.length() != 0) {
					...
					if (is != null) {
							...
							CapoApplication.logger.fine("Found a custom mime-mappings property defined by the property -Dmime-mappings ["
						...
						CapoApplication.logger.fine("Successfully loaded the mime mappings file from property -Dmime-mappings ["
								+ System.getProperty("mime-mappings") + "].");
					}
				}
			} catch (Exception ex) {
				CapoApplication.logger.log(Level.SEVERE,"Failed to load the mime-mappings file defined by the property -Dmime-mappings ["
						+ System.getProperty("mime-mappings") + "].");
			} finally {
				is = closeStream(is);
			}
